...
--✔8 StudentInfo(student, name, login, program, branch, finished, registered seminarCourses, mathCredits, researchCredits, totalCredits, canGraduate)

CREATE VIEW StudentInfo AS
WITH 
table1 AS (SELECT idnr AS student,name,login,program,branch FROM BasicInformation), 
table2 AS (SELECT student,course,grade,credits FROM finishedCourses),
table3 AS (SELECT student,course,status FROM Registrations),
table4 AS (SELECT student,seminarCourses, mathCredits, researchCredits, totalCredits,qualified AS canGraduate FROM pathToGraduation)
SELECT * 
FROM table1 NATURAL FULL OUTER JOIN table2 NATURAL FULL OUTER JOIN table3 NATURAL FULL OUTER JOIN table4;
